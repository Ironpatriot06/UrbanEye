# UrbanEye+ dataset status

Generated 2026-09-14T12:16:21+00:00

## Core datasets

| Dataset | Type | Acquisition | Raw files | Raw size | Processed rows | Licence status |
|---|---|---|---|---|---|---|
| sf311 | tabular | **AVAILABLE LOCALLY** | 1 | 150.5 MB | — | **VERIFIED** |
| boston311 | tabular | **NOT DOWNLOADED** | 0 | — | — | **VERIFIED** |
| chicago311 | tabular | **AVAILABLE LOCALLY** | 1 | 934.4 MB | — | **VERIFIED** |
| nyc311 | tabular | **AVAILABLE LOCALLY** | 1 | 72.0 MB | — | **VERIFIED** |

## Licences

| Dataset | Licence | Commercial | Redistribute | Decision |
|---|---|---|---|---|
| sf311 | Open Data Commons Public Domain Dedication and Licence (PDDL) 1.0 | True | True | **INCLUDE** |
| boston311 | Open Data Commons Public Domain Dedication and Licence (PDDL) | True | True | **INCLUDE** |
| chicago311 | City of Chicago Open Data Terms of Use | True | True | **INCLUDE** |
| nyc311 | NYC Open Data Terms of Use | True | True | **INCLUDE** |

## Deliberately excluded (documented, not silently dropped)

| Dataset | Licence | Decision | Reason |
|---|---|---|---|

## What this means for you

**Still to download (1):** boston311

See `DOWNLOAD_INSTRUCTIONS.md` for the exact URL, target directory and
command for each.
